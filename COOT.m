...
%                 R1=0.2+ 0.6*rand;                
   ...
   for i=1:size(CootPos,1)
    ...
    omega=0.001+((0.79-0.001)/(1+exp(0.5-1.2*l/Max_iter)));
    for i=1:size(LeaderPos,1)
         if rand<0.5
            R=-1+2*rand;
            R3=rand();
        ...
        if rand<0.5             
            ...
            Temp=B*R3.*cos(2*pi*R).*(gBest-LeaderPos(i,:))-omega.*gBest;   %/////////        
        end   
    ...
    disp(['Best function value at iteration ',num2str(l),' is : ',num2str(Convergence_curve(l))]);
    l = l + 1;
end
